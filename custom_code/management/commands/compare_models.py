...
from custom_code.models import (
    Event, StraightLineModel, PSPLModel, FSPLModel, DavenportFlareModel)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if event:
            ...
            if pspl.count() > 0 and davenport_flare.count() > 0:
                diagnostics.compare_model_goodness_of_fit(event, pspl[0], davenport_flare[0])
            if fspl.count() > 0 and davenport_flare.count() > 0:
                diagnostics.compare_model_goodness_of_fit(event, fspl[0], davenport_flare[0])
